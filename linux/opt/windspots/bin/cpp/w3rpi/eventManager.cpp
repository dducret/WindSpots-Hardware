...
void EventManager::logIt() {
  ...
  snprintf(currentTime, sizeof(currentTime), "%02d:%02d:%02d:%03d", t->tm_hour, t->tm_min, t->tm_sec, (int)tp.tv_usec/1000);
      ...
      switch(ev.eventType) {
        case w3rpi_EVENT_INIT:
          if(w3rpi_debug) {
            ...
            if(Singleton::get())
              std::cerr <<  "w3rpi EventManager::eventLoop w3rpi Started !" << std::endl;
          }
          break;
          ...
          if(s != NULL && s->isDecoded()) {
            ...
            if(windDirection != 0) {
              if(myEventManager->direction_correction > 0)
                windDirection += myEventManager->direction_correction;
